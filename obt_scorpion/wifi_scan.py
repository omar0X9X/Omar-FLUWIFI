...
def scan_networks(iface: str) -> list[dict]:
    ...
    for block in chunks:
        ...
        rows.append({
            ...
            "frequency_mhz": int(m_freq.group(1)) if m_freq else None,
            "channel": int(m_chan.group(1)) if m_chan else None,
            "security": _security(block),
            "pmf": _pmf(block),
            "wps_hint": "WPS:" in block.upper(),
        })
    rows.sort(key=lambda x: x["signal_dbm"] if x["signal_dbm"] is not None else -999, reverse=True)
    return rows
...
def print_networks(networks: list[dict]) -> None:
    ...
    print("─" * 106)
    print(f"{'SSID':<28} {'BSSID':<18} {'SIG':>7} {'CH':>4} {'SECURITY':<14} {'PMF':<9} {'WPS':<4}")
    print("─" * 106)
    for n in networks:
        sig = "?" if n["signal_dbm"] is None else f"{n['signal_dbm']:.0f}"
        ch = "?" if n["channel"] is None else str(n["channel"])
        print(
            f"{n['ssid'][:27]:<28} {n['bssid']:<18} {sig:>7} {ch:>4} "
            f"{n['security']:<14} {n['pmf']:<9} {'yes' if n['wps_hint'] else 'no':<4}"
        )
